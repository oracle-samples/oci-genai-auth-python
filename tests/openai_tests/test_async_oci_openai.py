...
async def test_create_response_async(httpx_mock, oci_async_openai_client):
    # ---- Arrange ----
    _set_mock_create_response(httpx_mock=httpx_mock)

    # ---- Act ----
    result = await oci_async_openai_client.responses.create(model=MODEL, input="Hi")
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("httpx_mock")
async def test_get_response_async(httpx_mock, oci_async_openai_client):
    # ---- Arrange ----
    _set_mock_get_response(httpx_mock=httpx_mock, response_id=RESPONSE_ID)

    # ---- Act ----
    result = await oci_async_openai_client.responses.retrieve(response_id=RESPONSE_ID)
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("httpx_mock")
async def test_delete_response_async(httpx_mock, oci_async_openai_client):
    # ---- Arrange ----
    _set_mock_delete_response(httpx_mock=httpx_mock, response_id=RESPONSE_ID)

    # ---- Act ----
    await oci_async_openai_client.responses.delete(response_id=RESPONSE_ID)
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("httpx_mock")
async def test_create_response_fc_tools_async(httpx_mock, oci_async_openai_client):
    # ---- Arrange ----
    _set_mock_create_response_with_fc_tools(httpx_mock=httpx_mock)

    # ---- Act ----
    result = await oci_async_openai_client.responses.create(
        model=MODEL, input="Hi", tools=function_tools
    )
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("httpx_mock")
async def test_create_response_file_input_async(httpx_mock, oci_async_openai_client):
    # ---- Arrange ----
...
@pytest.mark.asyncio
@pytest.mark.usefixtures("httpx_mock")
async def test_create_response_web_search_async(httpx_mock, oci_async_openai_client):
    # ---- Arrange ----
